...
class BotGUI(ctk.CTk):
    def __init__(self):
        ...
        self.token_entry = ctk.CTkEntry(self.left_frame, width=320)
        ...
        self.save_button = ctk.CTkButton(self.bottom_frame, text="Save Config", command=self.save_config_file)
    ...
    def update_active_channels_display(self):
        ...
        # Add a label for each active channel from config
    ...
    def save_config_file(self):
        ...
        if channel_id.isdigit():
            if channel_id not in self.config['channel_settings']:
                # Inherit from default and then override purpose
                self.config['channel_settings'][channel_id] = self.config['default_personality'].copy()
    ...
    def get_python_executable(self):
        """Finds the python executable, especially for virtual environments."""
    ...
    def start_bot(self):
        if self.bot_process is None or self.bot_process.poll() is not None:
            ...
            # Use Popen to run bot.py in a separate, non-blocking process
            self.bot_process = subprocess.Popen([python_exec, 'bot.py'])
